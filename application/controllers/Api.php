...
class Api extends REST_Controller 
{
 ...
 # return_fields comma separated name of fields
  ...
  public function entries_post($apikey=NULL)
  {
    ...
    if(isset($entry['creation_date'])) $entry_data['creation_date']=new MongoInt32($entry['creation_date']);   
    ...
    { 
      ...
      file_put_contents('debug.log',print_r(array("status" => "true", "id" => $response),TRUE),FILE_APPEND); 
      if(!empty($ouput)) $this->response(array("status" => "true", "id" => $response), REST_Controller::HTTP_OK);
